...
function FileChooser:getList(path, ...)
    if self._in_getlist and self:getVirtualPathTypePath(path) then
        return {}, {}, 0
    end

    self._in_getlist = true
    local res = table.pack(original_getList(self, path, ...))
    self._in_getlist = false
