...
function ReaderUI:showReader(file, provider, seamless, is_provider_forced, after_open_callback)
    ...
    if ok and BookshelfWidget.live and not G_reader_settings:readSetting("start_with") == "bookshelf" then
        ...
        if self.view.footer_visible then
            ...
            if self.document.configurable.t_page_margin ~= Screen:unscaleBySize(top_footer_height) or
                ...
                self.document.configurable.h_page_margins[2] ~= side_margins[2] then
                    ...
                    self.document.configurable.h_page_margins = side_margins
                    UIManager:sendEvent(Event:new("SetPageHorizMargins", side_margins))
                    UIManager:sendEvent(Event:new("SetPageTopMargin", Screen:unscaleBySize(top_footer_height)))
                    UIManager:sendEvent(Event:new("SetPageBottomMargin", Screen:unscaleBySize(bottom_footer_height)))
                else
                    self:showBookStatus()
                end
        else
