...
local Doublebar = require("apps/reader/modules/doublebar")
...
--            end
            ...
            -- We need scaled pixels and that's what we get with self.footer:getHeight()
    ...
    -- There is a reference in the ui for every plugin we can use
    if self.ui.pagetextinfo then
        self.ui.pagetextinfo:paintTo(bb, x, y)
    end
    ...
    --     local wpm_session = self.footer.ui.statistics.wpm_session
...
function ReaderView:drawXPointerVocabulary(bb, x, y)
    -- Getting screen boxes is done for each tap on screen (changing pages,
    -- showing menu...). We might want to cache these boxes per page (and
    -- clear that cache when page layout change or highlights are added
    -- or removed).
