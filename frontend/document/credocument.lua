...
function CreDocument:engineInit()
    if not engine_initialized then
        ...
        for k, v in ipairs(fonts) do
            if not v:find("/urw/") and not v:find("/nerdfonts/symbols.ttf") and not v:find("/usr/java/lib/fonts/") then -- and not v:find(".*AtkinsonHyperlegible.*") then -- and not v:find(".*Canela.*") then -- and not v:find(".*SourceSans3.*") then
...
function CreDocument:findAllText(pattern, case_insensitive, nb_context_words, max_hits, regex, search_flags, untilCurrentPage)
    logger.dbg("CreDocument: find all text", pattern, case_insensitive, regex, max_hits, true, nb_context_words, search_flags)
    return self._document:findAllText(pattern, case_insensitive, regex, max_hits, true, nb_context_words, search_flags, untilCurrentPage)
end
