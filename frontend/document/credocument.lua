...
function CreDocument:engineInit()
    if not engine_initialized then
        ...
        for k, v in ipairs(fonts) do
            if not v:find("/urw/") and not v:find("/nerdfonts/symbols.ttf") and not v:find("/usr/java/lib/fonts/") and not v:find(".*AtkinsonHyperlegible.*") then -- and not v:find(".*Canela.*") then -- and not v:find(".*SourceSans3.*") then
